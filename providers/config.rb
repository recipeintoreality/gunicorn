...
action :create do
  ...
  template_variables = {}
  %w{listen backlog preload_app worker_processes worker_class worker_timeout worker_keepalive worker_max_requests server_hooks pid accesslog access_log_format errorlog loglevel logger_class logconfig secure_scheme_headers forwarded_allow_ips proc_name syslog syslog_addr syslog_prefix syslog_facility}.each do |a|
